...
drawSpheres(void)
{
  ...
  if (raytracer.world.objectList->Length() > 0) {

	  /* Access and draw the objects in the list in sequence */
	  optr = raytracer.world.objectList->First();
	  while (optr != NULL) {
		  optr->Draw();
		  optr = raytracer.world.objectList->Next();
	  }
  }
}
...
main(int argc, char* argv[])
{
  ...
  if (argc >= 2) {
	  ...
	  raytracer.world.bbox = new BBox(raytracer.world.objectList);
  }
